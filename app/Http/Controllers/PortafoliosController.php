...
$carreraActual=DB::table("carrera")->join("portafolio","carrera.id","=", "portafolio.idCar")->where("portafolio.id","=",$idPor)->select("carrera.*")->get();


$periodoActual=DB::table("periodo")->join("portafolio","periodo.id","=","portafolio.idPer")->where("portafolio.id","=",$idPor)->select("periodo.*")->get();


$paralelo = DB::table('paralelo')->orderBy('nombre', 'asc')->get();
return view("Docente.materiasPortafolio")->with("carreraActual",$carreraActual)->with("periodoActual",$periodoActual)->with("idPortafolioActual",$idPor)->with('paralelo',$paralelo);
...
if (count($parametroMate)) {
  return view("Docente.parametrosAsignatura")->with("idPorMat",$idPorMat)->with("parametrosMateria",$parametroMate);
}else{
